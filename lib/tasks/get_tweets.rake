task :get_tweets => :environment do
	...
	while x < 200 and y < num_screen_names
		unless Tweet.exists?(:source => all_news_source_tweets[y][x].attrs[:id_str])
			Tweet.create(:screen_name => all_news_source_tweets[y][x].user.screen_name, :text => all_news_source_tweets[y][x].text, :timestamp => all_news_source_tweets[y][x].created_at, :source => all_news_source_tweets[y][x].attrs[:id_str])
		end
		x +=1
		if x == 199
			y += 1				
